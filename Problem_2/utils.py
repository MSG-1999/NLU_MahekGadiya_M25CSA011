...
class Logger:
    def __init__(self, path):
        ...
        self.log      = open(path, "a", buffering=1)   # append so each script adds to same log
...
class CharDataset(Dataset):
    """
    Encodes each string as [SOS, c1,...,cn, EOS].
    Input  = seq[:-1]   (teacher-forcing input)
    Target = seq[1:]    (what the model must predict)
    PAD=0 is ignored in the loss.
    """
...
def train_model(model, loader, epochs=80, optimizer=None,
    ...
    for ep in range(1, epochs + 1):
        ...
        for inp, tgt in loader:
            ...
            # Reversed-sequence augmentation (BLSTM only)
...
def is_valid(name, min_len=2, max_run=3):
    """Filter names too short, with long char runs, or >55% one char."""
    if len(name) < min_len:
        return False
    nl = name.lower()
...
def load_full_names(data_path="TrainingNames.txt"):
    """Load TrainingNames.txt as FULL name sequences (e.g. 'Rahul Sharma').
